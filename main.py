...
'''
Calculates Region of Interest(ROI) by receiving points from mouse event and transform perspective so that
we can have a top view of the scene or ROI. This top view or bird's eye view has the property that points are
distributed uniformly horizontally and vertically (scale for horizontal and vertical direction will be
different). So for bird's eye view, points are equally distributed, which was not the case for normal view.

YOLO V3 is used to detect humans in the frame and by calculating the bottom center point of bounding boxes around humans,
we transform those points to bird's eye view. And then calculates risk factor by calculating distance between
points and then drawing bird's eye view and drawing bounding boxes and distance lines between boxes on the frame.
'''
